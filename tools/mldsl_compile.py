...
IFPLAYER_RE = re.compile(r"^\s*ifplayer\s+([\w\u0400-\u04FF]+)\s*\((.*)\)\s*\{\s*$", re.I)
...
def compile_entries(path: Path) -> list[dict]:
    ...
    for raw in lines:
        ...
        if m_ifp:
            if not in_block:
                raise ValueError("ifplayer must be inside event/func/loop block")
            block_stack.append("if")
            func = (m_ifp.group(1) or "").strip()
            arg_str = m_ifp.group(2) or ""
            res = compile_line(api, f"if_player.{func}({arg_str})")
            if not res:
                raise ValueError(f"Unknown ifplayer condition: {func}")
            pieces, spec = res
